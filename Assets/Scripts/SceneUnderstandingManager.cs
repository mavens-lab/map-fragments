...
namespace Microsoft.MixedReality.SceneUnderstanding.Samples.Unity
{
    ...
    public class SceneUnderstandingManager : MonoBehaviour
    {
        ...
        public List<TextAsset> SUSerializedScenePaths;
        ...
        public List<GameObject> SceneRootList;
        ...
        private const int MaxMessages = 1;  //max number of SQS messages received at once
        private const int WaitTime = 2; //seconds between SQS poll requests
        private const String sqsAddress = "https://sqs.us-west-1.amazonaws.com/872711132385/map-fragments-queue";

        private bool FileLoaded = false;    //whether the scenes have been downloaded
        private bool FileDownloadStarted = false;
        private bool FileListLoaded = false;    //whether the list of files in the S3 bucket have been downloaded

        //[Tooltip("When enabled, new scenes will be periodically fetched from S3.")]
        private bool AutoUpdateSceneList;


        List<String> ObjectNames;   //the names of the scenes in the S3 bucket
        List<bool> ObjectLoaded;    //whether the corresponding scene has been loaded
        private int NumObjects = 0; //number of scenes
        private System.Diagnostics.Stopwatch stopwatch2;    //special use, only use once

        //MetadataFieldStringInterface SceneListInterface;
        ...
        {
            filename = Application.dataPath + "/LatencyLog.txt"; //JZ: for logging the latency Debugs because player log doesn't work.
            tw = new StreamWriter(filename, false); //clears the file contents of LatencyLog.txt
            tw.Close(); //make sure to close the file 
...
#pragma warning restore CS4014
            ...
            //JC: startup stuff
            //SceneListInterface = m_SceneList.GetComponent<MetadataFieldStringInterface>();
            ...
            AwsCredentials awsCredentials = GetComponent<AwsCredentials>(); 
            var credentials = new BasicAWSCredentials(awsCredentials.accessKey, awsCredentials.secretKey); // JZ: added the credentials back in
            s3Client = new AmazonS3Client(credentials, config);
            //sqsClient = new AmazonSQSClient(bucketRegion);
        
            //sqsClient = new AmazonSQSClient(credentials, sqsConfig); JZ: set up for later

            //s3Client = new AmazonS3Client(bucketRegion);

            //sqsClient = new AmazonSQSClient(bucketRegion);    //JC: to fix
            //await PurgeSQSQueue(); //purge SQS queue since we will reload all scenes anyway (needed in case Hololens uploaded scenes while client was offline, thus adding queue messages)    //JC: to fix


            stopwatch2 = new System.Diagnostics.Stopwatch();
            stopwatch2.Start();
            //m_SceneList.AURORA_Object.ObjectOwnershipAcquiredHandler += (o) =>
            //{
            //ParseSceneList(); //JC: 

            //};

            LoadFileList(); //JC: to load the current scene
        
            
            //m_SceneList.RequestOwnership();



            stopwatch.Stop();
            Debug.Log("JC: latency: startup: " + stopwatch.ElapsedMilliseconds);





            //to do: delete all the messages in SQS upon startup

            //load automatically
            //InvokeRepeating("LoadFileList", 0.0f, 5.0f);    //waits 1 second for object ownership to be granted

            //load from scene object metadata
        ...
        {
            //Debug.Log("JC: SceneUnderstandingManager: " + m_SceneList.name + m_SceneList.m_auroraObject);
            //Debug.Log("JC: SceneList isLocallyOwned: " + m_SceneList.IsLocallyOwned);
            ...
            else if (FileListLoaded && !FileDownloadStarted)
            {
            ...
            //Debug.Log("JC: IsLocallyOwned 0:" + SceneRootList[0].AURORA_Object.IsLocallyOwned);
            //Debug.Log("JC: IsLocallyOwned 1:" + SceneRootList[1].AURORA_Object.IsLocallyOwned);
            //Debug.Log("JC: IsLocallyOwned: " + m_SceneList.IsLocallyOwned);
            //Debug.Log("JC: ToggleState: " + AutoUpdateSceneList);
        }
        ...
        private IEnumerator DisplayDataRoutine(TaskCompletionSource<bool> completionSource)
        {
            ...
            SceneRootList = new List<GameObject>();
            ...
            Debug.Log("JC: DisplayDataRoutine: suScenes.Count " + suScenes.Count + ", SceneRootList.Count " + SceneRootList.Count);
            ...
            foreach (Scene suScene in suScenes)
            {
                if (suScene != null)
                {
                    ...
                    if (sceneToUnityTransformAsMatrix4x4 != null)
                    {
                        ...
                        //SceneRootList.Add(CreateAuroraSceneObject());
            ...
            stopwatch.Stop();
            Debug.Log("JC: latency: display data: " + stopwatch.ElapsedMilliseconds);
            ...
            if (!QuerySceneFromDevice)
            {
                ...
                if (suScene != null)
                {
                    ...
                    if (sceneToUnityTransformAsMatrix4x4 != null)
                    {
                        ...
                        if (!RunOnDevice)
                        {
                            //JC: commmented this out for now
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        #region Logging Debugs to a File
        string filename;
        TextWriter tw;
        ...
        private async Task LoadFileList()
        {
            ...
            ObjectNames = await ListingObjectsAsync(BucketName);



            //if (ObjectNames.Count != NumObjects && m_SceneList.IsLocallyOwned)   //check if the number of objects on the server has changed, and the scene object is locally owned (so that we can update its metadata)
            //{
            //    Debug.Log("JC: new # scenes:" + ObjectNames.Count);
            //    Debug.Log("JC: old # scenes:" + NumObjects);

            //    NumObjects = ObjectNames.Count;

                //if (SceneListInterface != null)
                //{
                    string ObjectNamesConcat = "";
                    foreach (string s in ObjectNames)
                    {
                        ObjectNamesConcat += s + ", ";
                    }
                    Debug.Log("JZ: " + ObjectNamesConcat);  
                    //SceneListInterface.Text = ObjectNamesConcat;
                    //Debug.Log("JC: SceneList metadata: " + SceneListInterface.Text);
                //}
                //JC: to do: currently loads all files again

                //ResetScene(); //JZ: commented out for now, since scenes not auto-updated from S3, so no need to clear
                FileListLoaded = true;

            stopwatch.Stop();
            Debug.Log("JC: latency: download file list S3: " + stopwatch.ElapsedMilliseconds);
        ...
        private void ResetScene()
        {
            ...
            DisplayFromDiskStarted = false;
            SUSerializedScenePaths = new List<TextAsset>(0);
        ...
        private async Task LoadFileS3()
        {
            ...
            stopwatch.Stop();
            Debug.Log("JC: latency: download file S3: " + stopwatch.ElapsedMilliseconds);
        ...
        {
            Debug.Log("JC: ReadObjectDataAddFragmentsAsync: " + ObjectName);
            try
            {
                ...
                stopwatch.Stop();
                Debug.Log("JC: latency: download file S3 direct: " + stopwatch.ElapsedMilliseconds);
                ...
                using (BinaryReader reader = new BinaryReader(responseStream))
                {
                    ...
                    stopwatch5.Stop();
                    Debug.Log("JC: latency: read network data: " + stopwatch5.ElapsedMilliseconds);
                    ...
                    stopwatch3.Stop();
                    Debug.Log("JC: latency: write file: " + stopwatch3.ElapsedMilliseconds);
                    ...
                    */
                    ...
                    //JC: load saved .bytes file from disk using AssetDatabase
                    ...
                    stopwatch4.Stop();
                    Debug.Log("JC: latency: load file: " + stopwatch4.ElapsedMilliseconds);

                    SUSerializedScenePaths.Add(myTextAsset);

                    stopwatch2.Stop();
                    Debug.Log("JC: latency: download file save/load: " + stopwatch2.ElapsedMilliseconds);
        ...
        static async Task<List<string>> ListingObjectsAsync(string BucketName)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    {
                        if (entry.Key.Contains("/")) //JZ: this condition will prevent the folder from being added to the list of S3Object
                        {
                            continue;
                        }
        ...
        public void TakeOwnershipAfterSpawn(AURORA.ClientAPI.AuroraObject auroraObject, AURORA.ClientAPI.RpcRequest rpcRequest)
        {
            ...
            Debug.Log("JC: TakeOwnershipAfterSpawn");


        }


        public void ChangeMetadataText(AURORA.ClientAPI.AuroraObject auroraObject, AURORA.ClientAPI.RpcRequest rpcRequest)
        {
            Debug.Log("JC: ChangeMetadataText");
            //m_metadataFieldStringInterface.Text = "Hello world";
            //Debug.Log("JC: modified text");

            //MetadataFieldStringInterface SceneListInterface = m_SceneList.GetComponent<MetadataFieldStringInterface>();
            //if (SceneListInterface != null)
            //{
                //JC: bug when using .Text, something about the key in MetadatInterface
            //    SceneListInterface.Text = "Hello world!";
            //    Debug.Log("JC: SceneList metadata: " + SceneListInterface.Text);
            //}
        }

        private GameObject CreateAuroraSceneObject()
        {
            System.Diagnostics.Stopwatch stopwatch = new System.Diagnostics.Stopwatch();
            stopwatch.Start();
            ...
            //var ago = newobject.GetComponent<GameObject>();
            //if (ago != null)
            //{
            ...
            Debug.Log("JC: added " + newobject + " to scene list");
            ...
            stopwatch.Stop();
            Debug.Log("JC: latency: create Aurora game object: " + stopwatch.ElapsedMilliseconds);

            return newobject;
        }

        private void ParseSceneList()
        {
            Debug.Log("JC: ParseSceneList");
            ...
            //if (SceneListInterface != null)
            //{
                //Debug.Log("JC: SceneListInterface metadata: " + SceneListInterface.Text);
                //string[] ObjectNamesString = SceneListInterface.Text.Split(',');


                //foreach (string s in ObjectNamesString)
                //{
                //    if (s != "")
                //    {
                //        ObjectNames.Add(s);
                //        Debug.Log("JC: ParseSceneList: " + s);
                //    }
            ...
            stopwatch.Stop();
            Debug.Log("JC: latency: load file list Aurora metadata: " + stopwatch.ElapsedMilliseconds);

            if (stopwatch2.IsRunning)
            {
                stopwatch2.Stop();
                Debug.Log("JC: latency: load file list Aurora metadata including ownership: " + stopwatch2.ElapsedMilliseconds);
            }
        ...
        private async Task PollSQS()
        {
            Debug.Log("JC: PollSQS");
            ...
            //purge whatever's in the queue already, since we just re-loaded everything
            //await PurgeSQSQueue();    //commented out this line since it hangs sometimes. As long as nobody uploads manually to Hololens while the client is running, it should be OK


            Debug.Log("JC: PollSQS: AutoUpdateSceneList: " + AutoUpdateSceneList);
            while (AutoUpdateSceneList)
            {
                Debug.Log("JC: checking for messages");
                var msg = await GetMessage(sqsClient, sqsAddress, WaitTime);
                if (msg.Messages.Count != 0)
                {
                    Debug.Log("JC: have message");
                    if (ProcessMessage(msg.Messages[0]))
                        await DeleteMessage(sqsClient, msg.Messages[0], sqsAddress);

                    //JC: if the SQS poll has a message, it means one of the objects changes. Therefore, reload all the scenes. To Do: only reload the changed scene
                    await LoadFileList();
        ...
        {
            Debug.Log($"JC: Message body of {message.MessageId}:");
            Debug.Log($"{message.Body}");
            int first = message.Body.IndexOf("key");
            int last = message.Body.IndexOf("size");
            String ObjectName = message.Body.Substring(first + 6, last - first - 9);
            Debug.Log($"JC: SQS notified that {ObjectName} changed");
            return true;
        }
        ...
        public void SetAutoUpdateState()
        {
            Debug.Log("JC: SetAutoUpdateState");
            GameObject toggle = GameObject.Find("Toggle_AutoUpdate");
            Toggle m_Toggle = toggle.GetComponent<Toggle>();
            if (m_Toggle != null) { 
                AutoUpdateSceneList = m_Toggle.isOn;
            }

            if (AutoUpdateSceneList)
            {
                Debug.Log("JC: toggle on");

                LoadFileList(); //reload the current scene
                PollSQS();  //start polling SQS
                //InvokeRepeating("LoadFileList", 0.0f, 5.0f); 
            }
            else
            {
                //CancelInvoke("LoadFileList");

                Debug.Log("JC: toggle off");
                ResetScene();
                ParseSceneList();
            }
        }

        private async Task PurgeSQSQueue()
        {
            Debug.Log("JC: PurgeSQSQueue");

            PurgeQueueRequest pq = new PurgeQueueRequest();
            pq.QueueUrl = sqsAddress;
            await sqsClient.PurgeQueueAsync(pq);

            Debug.Log("JC: PurgeSQSQueue finished");
        }
    }
